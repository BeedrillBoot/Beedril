...
Bot.on('pm', (by, message) => {
	...
	if (!message.startsWith(prefix)) {
		if (!Bot.auth.admin.includes(toId(by))) {
			...
			return Bot.pm(by, `Hi, I'm ${Bot.status.nickName}! I'm a Bot made by Beedril Boot and Dinesh. My prefix is \`\`${prefix}\`\`. For an overview of my commands, use \`\`${prefix}commands\`\`.`);
		}
		return;
	}
